...
const reComponentName: ReComponentName = (title) => {
  // console.log(title, 'titletitletitletitle') // Cascader 级联选择器
  // return `el-${hyphenate(removeTag(title)).replace(/[ ]+/g, '-')}`
  return `y-${hyphenate(removeTag(title)).replace(/[ ]+/g, '-')}`
}
...
export const buildHelper: TaskFunction = (done) => {
  ...
  main({
    name: name!,
    version: _version,
    // components-helper 内部是调用了fast-glob => fg.sync(_options.entry, _options.fastGlobConfig); 来检索文件的
    entry: '!(datetime-picker|message-box|message|test).md',
    fastGlobConfig: {
      cwd: path.resolve(projRoot, 'docs/zh-CN/component'), // 搜索文件目录
      absolute: true, // 绝对路径
    },
    outDir: epOutput,
    reComponentName,
